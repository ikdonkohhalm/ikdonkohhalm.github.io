...
( function () {
	...
	var level = 19; // starting/current level
	...
	let loops = 0;
	let mCode = null;
	let aCodes = []; //stores play codes for sounds
	let timer = 0;
	let over = false;
	PS.audioLoad("base_noise", { path : "audio/"});
    PS.audioLoad("supp_noise", { path : "audio/"});
    PS.audioLoad("outro", { path : "audio/"});
    PS.keyRepeat(true, 60, 60);
	...
	W = {
			...
			if ( data === IS_GOAL ) {
				...
				if ( ( level + 1 ) < len ) {
                    ...
                    switch (loops) {
						...
						case 4 :
							aCodes.push(PS.audioPlay("base_noise", { path : "audio/", onEnd : W.writhe}));
							break;
						...
						default :
							if (loops > 4) {
								if (aCodes.length < 7) {
                                    aCodes.push(PS.audioPlay("supp_noise", {path: "audio/"}));
                                }
							}
					}
		...
		despair : function() {
			...
			timer = PS.timerStart(1, automove);
			let ticks = 0;
		...
		writhe : function() {
			...
			PS.keyDown = function() {};
			PS.audioPlay("outro", { path : "audio/", onEnd : W.fadeStatus});
